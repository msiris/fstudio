...
세 모드 모두 **얼굴 유지** 토글이 있고 기본값은 켜짐이다
(Image Gen은 참조 이미지를 넣었을 때만 나타난다).

켜면 프롬프트에 강한 보존 문단이 들어간다. 바꾸면 안 되는 부위를 하나씩 세고,
미화·보정·나이 변경까지 따로 금지한다. "옷만 바꿔줘"라고 했는데 얼굴이 달라지는 것을 막기 위해서다.
참조 이미지 안내도 "참고해서 새로 구성하라"가 아니라
"지시가 말한 것만 가져오고 얼굴·배경·구도는 가져오지 마라"로 바뀐다.

얼굴 자체를 바꾸는 지시와는 양립할 수 없으므로, **얼굴 바꾸기 프리셋을 누르면 자동으로 꺼진다.**
직접 끄면 조건부 보존 문구로 내려간다.
...
- 얼굴 유지가 켜지면 `IDENTITY_LOCK` 문단을 넣는다. 바꾸면 안 되는 부위를 하나씩 세고
  미화·보정을 따로 금지한다. 짧게 "얼굴을 유지하라"고만 쓰면 모델이 미화를 유지의 범주로 본다
- 얼굴 유지를 끄면 조건부 문구(`IDENTITY_SOFT`)로 내려가 얼굴 교체 지시를 막지 않는다
- 참조 이미지가 붙고 얼굴 유지가 켜지면, 생성 프롬프트도 "새로 만들라"가 아니라
